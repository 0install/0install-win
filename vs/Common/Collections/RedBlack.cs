...
namespace Common.Collections
{
    ...
    internal class RedBlackTree<T> : IEnumerable<T>
    {
        ...
        private Node[] GetNodeStack()
        {
            ...
            if (count < 0x400)
                maxDepth = 21;
            else maxDepth = count < 0x10000 ? 41 : 65;
        ...
        private class Node
        {
            ...
            /// level.
            /// </summary>
            public int Count { get { return (int)(count & ~REDMASK); } set { count = (count & REDMASK) | (uint)value; } }
            ...
            public Node Clone()
            {
                var newNode = new Node {item = item, count = count};
        ...
        private void CheckEnumerationStamp(int startStamp)
        {
            ...
            {
                throw new InvalidOperationException(Resources.ChangeDuringEnumeration);
            }
        }
        ...
        /// </summary>
        public int ElementCount { get { return count; } }
        ...
        {
            var newTree = new RedBlackTree<T>(comparer) {count = count};
            if (root != null)
        ...
        public T GetItemByIndex(int index)
        {
            ...
            for (;;)
            {
                ...
                leftCount = current.left != null ? current.left.Count : 0;
        ...
        /// <returns><see langword="false"/> if duplicate exists, otherwise true.</returns>
                ...
                if (compare == 0)
                {
                    ...
                    if (dupPolicy == DuplicatePolicy.DoNothing)
                    {
                        ...
                        for (int i = 0; i < nodeStackPtr; ++i)
                            if (nodeStack != null) nodeStack[i].DecrementCount();
        ...
        /// <returns><see langword="true"/> if an element was deleted, <see langword="false"/> if no element had 
        ...
        public RangeTester BoundedRangeTester(bool useFirst, T first, bool useLast, T last)
        {
            return item =>
            {
                if (useFirst && comparer.Compare(first, item) > 0) return -1; // item is before first.
                else if (useLast && comparer.Compare(last, item) <= 0) return 1; // item is after or equal to last.
                else return 0; // item is greater or equal to first, and less than last.
            };
        }
        ...
        public RangeTester DoubleBoundedRangeTester(T first, bool firstInclusive, T last, bool lastInclusive)
        {
            return item =>
            {
                if (firstInclusive)
                {
                    if (comparer.Compare(first, item) > 0) return -1; // item is before first.
                }
                else if (comparer.Compare(first, item) >= 0) return -1; // item is before or equal to first.

                if (lastInclusive)
                {
                    if (comparer.Compare(last, item) < 0) return 1; // item is after last.
                }
                else if (comparer.Compare(last, item) <= 0) return 1; // item is after or equal to last
        ...
        public RangeTester LowerBoundedRangeTester(T first, bool inclusive)
        {
            return item =>
            {
                ...
                {
                    if (comparer.Compare(first, item) >= 0) return -1; // item is before or equal to first.
        ...
        public RangeTester UpperBoundedRangeTester(T last, bool inclusive)
        {
            return item =>
            {
                ...
                {
                    if (comparer.Compare(last, item) <= 0) return 1; // item is after or equal to last
        ...
        /// </summary>
        /// <param name="rangeTester">Tests an item against the custom range.</param>
        /// <param name="node">Node to begin enumeration; may be <see langword="null"/>.</param>
        ...
        /// <param name="item">Returns the item that was deleted, if true returned.</param>
        /// <returns><see langword="true"/> if an element was deleted, <see langword="false"/> if the range is empty.</returns>
        ...
        /// </summary>
        /// <param name="node">Sub-tree to validate; may be <see langword="null"/>.</param>
