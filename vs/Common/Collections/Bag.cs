...
namespace Common.Collections
{
    ...
    public class Bag<T> : BaseCollection<T>, ICloneable
    {
        ...
        public IEqualityComparer<T> Comparer { get { return keyEqualityComparer; } }
        ...
        public override sealed int Count { get { return count; } }
        ...
        public override sealed bool Remove(T item)
        {
            ...
            if (hash.Delete(NewPair(item), out removed))
            {
                if (removed.Value > 1)
                {
                    ...
                    KeyValuePair<T, int> dummy;
                    KeyValuePair<T, int> newPair = NewPair(removed.Key, removed.Value - 1);
                    hash.Insert(newPair, true, out dummy);
                }
                --count;
                return true;
        ...
        public void DifferenceWith(Bag<T> otherBag)
        {
            ...
            foreach (T item in otherBag.DistinctItems())
            {
                int copiesInThis = NumberOfCopies(item);
                int copiesInOther = otherBag.NumberOfCopies(item);
                int copies = copiesInThis - copiesInOther;
        ...
        public void SymmetricDifferenceWith(Bag<T> otherBag)
        {
            ...
            foreach (T item in otherBag.DistinctItems())
            {
                int copiesInThis = NumberOfCopies(item);
                int copiesInOther = otherBag.NumberOfCopies(item);
                int copies = Math.Abs(copiesInThis - copiesInOther);
