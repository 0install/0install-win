...
namespace Common.Collections
{
    ...
    [Serializable]
    public class OrderedMultiDictionary<TKey, TValue> : BaseMultiDictionary<TKey, TValue>, ICloneable
    {
        ...
        /// <param name="firstInclusive">True if the lower bound is inclusive, <see langword="false"/> if exclusive.</param>
        /// <param name="last">The upper bound.</param>
        /// <param name="lastInclusive">True if the upper bound is inclusive, <see langword="false"/> if exclusive.</param>
        ...
        private RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester DoubleBoundedKeyRangeTester(TKey first, bool firstInclusive, TKey last, bool lastInclusive)
        {
            return pair =>
            {
                if (firstInclusive)
                {
                    if (keyComparer.Compare(first, pair.Key) > 0) return -1; // item is before first.
                }
                else if (keyComparer.Compare(first, pair.Key) >= 0) return -1; // item is before or equal to first.

                if (lastInclusive)
                {
                    if (keyComparer.Compare(last, pair.Key) < 0) return 1; // item is after last.
                }
                else if (keyComparer.Compare(last, pair.Key) <= 0) return 1; // item is after or equal to last
        ...
        /// </summary>
        /// <param name="first">The lower bound.</param>
        /// <param name="inclusive">True if the lower bound is inclusive, <see langword="false"/> if exclusive.</param>
        /// <returns>A RangeTester delegate that tests for a key in the given range.</returns>
        private RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester LowerBoundedKeyRangeTester(TKey first, bool inclusive)
        {
            return pair =>
            {
                ...
                {
                    if (keyComparer.Compare(first, pair.Key) > 0) return -1; // item is before first.
                    else return 0; // item is after or equal to first
                }
                else
                {
                    if (keyComparer.Compare(first, pair.Key) >= 0) return -1; // item is before or equal to first.
                    else return 0; // item is after first
                }
            };
        }
        ...
        /// </summary>
        /// <param name="last">The upper bound.</param>
        /// <param name="inclusive">True if the upper bound is inclusive, <see langword="false"/> if exclusive.</param>
        /// <returns>A RangeTester delegate that tests for a key in the given range.</returns>
        private RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester UpperBoundedKeyRangeTester(TKey last, bool inclusive)
        {
            return pair =>
            {
                ...
                {
                    if (keyComparer.Compare(last, pair.Key) < 0) return 1; // item is after last.
                    else return 0; // item is before or equal to last.
                }
                else
                {
                    if (keyComparer.Compare(last, pair.Key) <= 0) return 1; // item is after or equal to last
                    else return 0; // item is before last.
                }
            };
        }
        ...
        /// <returns><see langword="true"/> if <paramref name="value"/> was associated with <paramref name="key"/> (and was
        /// therefore removed). <see langword="false"/> if <paramref name="value"/> was not associated with <paramref name="key"/>.</returns>
        ...
        /// <returns><see langword="true"/> if the key was present and was removed. Returns 
        /// <see langword="false"/> if the key was not present.</returns>
        ...
        public IComparer<TKey> KeyComparer { get { return keyComparer; } }
        ...
        public IComparer<TValue> ValueComparer { get { return valueComparer; } }
        ...
        public override sealed int Count { get { return keyCount; } }
        ...
        /// <returns><see langword="true"/> if <paramref name="value"/> is associated with <paramref name="key"/>.</returns>
        ...
        /// </summary>
        /// <param name="key">The key to check.</param>
        /// <returns><see langword="true"/> if <paramref name="key"/> is present and has at least
        ...
        private IEnumerator<TKey> EnumerateKeys(RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester rangeTester, bool reversed)
        {
            ...
            pairs = reversed ? tree.EnumerateRangeReversed(rangeTester) : tree.EnumerateRange(rangeTester);
        ...
        {
            throw new InvalidOperationException(string.Format(CultureInfo.CurrentUICulture, Resources.TypeNotCloneable, t.FullName));
        }
        ...
        /// with the key.
        /// </summary>
        public override sealed ICollection<KeyValuePair<TKey, TValue>> KeyValuePairs { get { return new KeyValuePairsCollection(this); } }
        ...
        /// </summary>
        [Serializable]
        private sealed class KeyValuePairsCollection : BaseReadOnlyCollection<KeyValuePair<TKey, TValue>>
        {
            ...
            public override int Count { get { return myDictionary.CountAllValues(); } }
        ...
        public ViewMultiDictionary Reversed()
        {
            return new ViewMultiDictionary(this, tree.EntireRangeTester, true, true);
        }
        ...
        public ViewMultiDictionary Range(TKey from, bool fromInclusive, TKey to, bool toInclusive)
        {
            return new ViewMultiDictionary(this, DoubleBoundedKeyRangeTester(from, fromInclusive, to, toInclusive), false, false);
        }
        ...
        /// be included in the range.</param>
        /// <returns>An OrderedMultiDictionary.View of key-value pairs in the given range.</returns>
        public ViewMultiDictionary RangeFrom(TKey from, bool fromInclusive)
        {
            return new ViewMultiDictionary(this, LowerBoundedKeyRangeTester(from, fromInclusive), false, false);
        }
        ...
        /// be included in the range.</param>
        /// <returns>An OrderedMultiDictionary.View of key-value pairs in the given range.</returns>
        public ViewMultiDictionary RangeTo(TKey to, bool toInclusive)
        {
            return new ViewMultiDictionary(this, UpperBoundedKeyRangeTester(to, toInclusive), false, false);
        }
        #endregion Views
        ...
        ///</remarks>
        [Serializable]
        [SuppressMessage("Microsoft.Design", "CA1034:NestedTypesShouldNotBeVisible", Justification = "Since this type can only be created by its parent type, nesting represents a sensible way of keeping the namespace uncluttered")]
        public class ViewMultiDictionary : BaseMultiDictionary<TKey, TValue>
        {
            private readonly OrderedMultiDictionary<TKey, TValue> myDictionary;
            ...
            internal ViewMultiDictionary(OrderedMultiDictionary<TKey, TValue> myDictionary, RedBlackTree<KeyValuePair<TKey, TValue>>.RangeTester rangeTester, bool entireTree, bool reversed)
            {
                this.myDictionary = myDictionary;
            ...
            /// <returns><see langword="true"/> if the key is within the bounds of this view.</returns>
            ...
            /// <returns><see langword="true"/> if the key exists and has values associated with it. False otherwise.</returns>
            protected override sealed bool TryEnumerateValuesForKey(TKey key, out IEnumerator<TValue> values)
            {
                if (!KeyInView(key))
                {
            ...
            public override sealed int Count
            {
                get
                {
                    ...
                    else
                    {
                        ...
                        using (var enumKeys = myDictionary.EnumerateKeys(rangeTester, reversed))
                        {
            ...
            /// <returns><see langword="true"/> if the key is within this view. </returns>
            public override sealed bool ContainsKey(TKey key)
            {
                if (!KeyInView(key))
                    return false;
            ...
            /// <returns><see langword="true"/> if the key-value pair is within this view. </returns>
            public override sealed bool Contains(TKey key, TValue value)
            {
                if (!KeyInView(key))
                    return false;
            ...
            {
                if (!KeyInView(key))
                    throw new ArgumentException(Resources.OutOfViewRange, "key");
                else
            ...
            /// </summary>
            /// <param name="key">The key to remove.</param>
            /// <returns><see langword="true"/> if the key was found and removed. <see langword="false"/> if the key was not found.</returns>
            public override sealed bool Remove(TKey key)
            {
                if (!KeyInView(key))
                    return false;
            ...
            /// <param name="key">The key to remove.</param>
            /// <param name="value">The value to remove.</param>
            /// <returns><see langword="true"/> if the key-value pair was found and removed. <see langword="false"/> if the key-value pair was not found.</returns>
            public override sealed bool Remove(TKey key, TValue value)
            {
                if (!KeyInView(key))
                    return false;
            ...
            public ViewMultiDictionary Reversed()
            {
                return new ViewMultiDictionary(myDictionary, rangeTester, entireTree, !reversed);
            }
        }
    }
}