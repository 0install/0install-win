...
namespace Common.Collections
{
    ...
    [SuppressMessage("Microsoft.Design", "CA1036:OverrideMethodsOnComparableTypes", Justification = "Comparison is only done by sorting algorithms")]
    public struct Triple<TFirst, TSecond, TThird> : IComparable, IComparable<Triple<TFirst, TSecond, TThird>>
    {
        ...
        /// <returns><see langword="true"/> if the objects are equal. <see langword="false"/> if the objects are not equal.</returns>
        public override bool Equals(object obj)
        {
            if (obj != null && obj is Triple<TFirst, TSecond, TThird>)
            {
                var other = (Triple<TFirst, TSecond, TThird>)obj;
        ...
        /// <returns><see langword="true"/> if the triples are equal. <see langword="false"/> if the triples are not equal.</returns>
        ...
        public int CompareTo(Triple<TFirst, TSecond, TThird> other)
        {
            ...
            catch (ArgumentException)
            {
                ...
                {
                    throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.UncomparableType, typeof(TFirst).FullName));
                }
                else if (!typeof(IComparable<TSecond>).IsAssignableFrom(typeof(TSecond)) &&
                         !typeof(IComparable).IsAssignableFrom(typeof(TSecond)))
                {
                    throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.UncomparableType, typeof(TSecond).FullName));
                }
                else if (!typeof(IComparable<TThird>).IsAssignableFrom(typeof(TThird)) &&
                         !typeof(IComparable).IsAssignableFrom(typeof(TThird)))
                {
                    throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.UncomparableType, typeof(TThird).FullName));
                }
                else
        ...
        int IComparable.CompareTo(object obj)
        {
            if (obj is Triple<TFirst, TSecond, TThird>)
                ...
                throw new ArgumentException(Resources.BadComparandType, "obj");
        }
        ...
        {
            return string.Format(CultureInfo.InvariantCulture, "First: {0}, Second: {1}, Third: {2}",
        ...
        /// <param name="pair1">First triple to compare.</param>
        /// <param name="pair2">Second triple to compare.</param>
        /// <returns><see langword="true"/> if the triples are not equal. <see langword="false"/> if the triples are equal.</returns>
        public static bool operator !=(Triple<TFirst, TSecond, TThird> pair1, Triple<TFirst, TSecond, TThird> pair2)
        {
            return ! pair1.Equals(pair2);
        }
