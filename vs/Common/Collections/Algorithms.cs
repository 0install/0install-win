...
namespace Common.Collections
{
    ...
    /// items of the same type, and returns a boolean value representationg 
    ...
    public static class Algorithms
    {
        ...
        /// </summary>
        [Serializable]
        private class ListRange<T> : BaseList<T>, ICollection<T>
        {
            ...
            public override int Count { get { return Math.Min(count, wrappedList.Count - start); } }
            ...
            public override bool Remove(T item)
            {
                if (wrappedList.IsReadOnly)
                    throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.CannotModifyCollection, "Range"));
                else
            ...
            bool ICollection<T>.IsReadOnly { get { return wrappedList.IsReadOnly; } }
        }
        ...
        /// </summary>
        [Serializable]
        private class ArrayRange<T> : BaseList<T>
        {
        ...
        private class ReadOnlyCollection<T> : ICollection<T>
        {
            ...
            public int Count { get { return wrappedCollection.Count; } }

            public bool IsReadOnly { get { return true; } }

            public void Add(T item)
            {
                throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.CannotModifyCollection, "read-only collection"));
            }
        ...
        private class ReadOnlyList<T> : IList<T>
        {
            ...
            public T this[int index] { get { return wrappedList[index]; } set { throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.CannotModifyCollection, "read-only list")); } }

            public void Add(T item)
            {
                throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.CannotModifyCollection, "read-only list"));
            }
        ...
        private class ReadOnlyDictionary<TKey, TValue> : IDictionary<TKey, TValue>
        {
            ...
            {
                throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.CannotModifyCollection, "read-only dictionary"));
            }
            ...
            public ICollection<TKey> Keys { get { return ReadOnly(wrappedDictionary.Keys); } }

            public ICollection<TValue> Values { get { return ReadOnly(wrappedDictionary.Values); } }
            ...
            public TValue this[TKey key] { get { return wrappedDictionary[key]; } set { throw new NotSupportedException(string.Format(CultureInfo.CurrentUICulture, Resources.CannotModifyCollection, "read-only dictionary")); } }
            ...
            public int Count { get { return wrappedDictionary.Count; } }
        ...
        private class TypedEnumerator<T> : IEnumerator<T>
        {
            ...
            T IEnumerator<T>.Current { get { return (T)wrappedEnumerator.Current; } }

            void IDisposable.Dispose()
            {
                var disposableEnumerator = wrappedEnumerator as IDisposable;
                if (disposableEnumerator != null) disposableEnumerator.Dispose();

                GC.SuppressFinalize(this);
            }

            object IEnumerator.Current { get { return wrappedEnumerator.Current; } }
        ...
        public static IEnumerable<T> TypedAs<T>(IEnumerable untypedCollection)
        {
            ...
            {
                var enumerableCollection = untypedCollection as IEnumerable<T>;
                if (enumerableCollection == null) return new TypedEnumerable<T>(untypedCollection);
                else return enumerableCollection;
            }
        }
        ...
        public static ICollection<T> TypedAs<T>(ICollection untypedCollection)
        {
            ...
            {
                var typedCollection = untypedCollection as ICollection<T>;
                if (typedCollection == null) return new TypedCollection<T>(untypedCollection);
                else return typedCollection;
            }
        }
        ...
        private class TypedList<T> : IList<T>
        {
            ...
            public T this[int index] { get { return (T)wrappedList[index]; } set { wrappedList[index] = value; } }

            public int Count { get { return wrappedList.Count; } }

            public bool IsReadOnly { get { return wrappedList.IsReadOnly; } }
        ...
        public static IList<T> TypedAs<T>(IList untypedList)
        {
            ...
            {
                var typedList = untypedList as IList<T>;
                if (typedList == null) return new TypedList<T>(untypedList);
                else return typedList;
            }
        }
        ...
        private class UntypedCollection<T> : ICollection
        {
            ...
            public void CopyTo(Array array, int index)
            {
                ...
                if (index < 0)
                    throw new ArgumentOutOfRangeException("index", index, Resources.ArgMustNotBeNegative);
                if (index >= array.Length || count > array.Length - index)
                    throw new ArgumentException(Resources.ArrayTooSmall, "index");
        ...
        public static ICollection Untyped<T>(ICollection<T> typedCollection)
        {
            ...
            {
                var genericCollection = typedCollection as ICollection;
        ...
        private class UntypedList<T> : IList
        {
            ...
            private static T ConvertToItemType(string name, object value)
            {
                ...
                {
                    throw new ArgumentException(string.Format(CultureInfo.CurrentUICulture, Resources.WrongType, value, typeof(T)), name);
                }
            }
            ...
            public object this[int index] { get { return wrappedList[index]; } set { wrappedList[index] = ConvertToItemType("value", value); } }
        ...
        public static IList Untyped<T>(IList<T> typedList)
        {
            ...
            {
                var genericList = typedList as IList;
                if (genericList == null)
        ...
        /// </summary>
        [Serializable]
        private class ArrayWrapper<T> : BaseList<T>, IList
        {
            private readonly T[] wrappedArray;
            ...
            public override int Count { get { return wrappedArray.Length; } }
        ...
        /// <param name="itemFind">The value to find and replace within the collection</param>
        ...
        /// <param name="itemFind">The value to find and replace within <typeparamref name="T"/>.</param>
        /// <param name="replaceWith">The new value to replace with.</param>
        ...
        public static void ReplaceInPlace<T>(IList<T> list, T itemFind, T replaceWith, IEqualityComparer<T> equalityComparer)
        {
            ...
            var typedArray = list as T[];
            if (typedArray != null)
                list = new ArrayWrapper<T>(typedArray);
            if (list.IsReadOnly)
                throw new ArgumentException(Resources.ListIsReadOnly, "list");
        ...
        public static void RemoveDuplicatesInPlace<T>(IList<T> list, BinaryPredicate<T> predicate)
        {
            ...
            if (i < listCount)
            {
                ...
                var genericCollection = list as IList;
                if (list is ArrayWrapper<T> || (genericCollection != null && genericCollection.IsFixedSize))
                {
        ...
        /// <returns><see langword="true"/> if an item satisfying the condition was found. <see langword="false"/> if no such item exists in the collection.</returns>
        ...
        /// <summary>
        /// Enumerates the indexes of all the items in <paramref name="list"/> that satisfy the condition defined
        /// by <paramref name="predicate"/>.
        /// </summary>
        /// <param name="list">The list to check all the items in.</param>
        /// <param name="predicate">A delegate that defines the condition to check for.</param>
        /// <returns>An IEnumerable&lt;T&gt; that enumerates the indexes of items that satisfy the condition.</returns>
        public static IEnumerable<int> FindIndexesWhere<T>(IList<T> list, Predicate<T> predicate)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (predicate == null)
        ...
        /// <summary>
        /// Enumerates the indexes of all the items in a list equal to a given item.
        /// </summary>
        /// <remarks>The default sense of equality for T is used, as defined by T's
        /// implementation of IComparable&lt;T&gt;.Equals or object.Equals.</remarks>
        /// <param name="list">The list to search.</param>
        /// <param name="item">The item to search for.</param>
        /// <returns>An IEnumerable&lt;T&gt; that enumerates the indexes of items equal to <paramref name="item"/>. </returns>
        public static IEnumerable<int> IndexesOf<T>(IList<T> list, T item)
        {
            return IndexesOf(list, item, EqualityComparer<T>.Default);
        }

        /// <summary>
        /// Enumerates the indexes of all the items in a list equal to a given item. A passed
        /// IEqualityComparer is used to determine equality.
        /// </summary>
        /// <param name="list">The list to search.</param>
        /// <param name="item">The item to search for.</param>
        /// <param name="equalityComparer">The IEqualityComparer&lt;T&gt; used to compare items for equality. Only the Equals method will be called.</param>
        /// <returns>An IEnumerable&lt;T&gt; that enumerates the indexes of items equal to <paramref name="item"/>. </returns>
        public static IEnumerable<int> IndexesOf<T>(IList<T> list, T item, IEqualityComparer<T> equalityComparer)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (equalityComparer == null)
        ...
        /// <summary>
        /// Enumerates the indexes of all the items in a list equal to one of several given items. 
        /// </summary>
        /// <remarks>The default sense of equality for T is used, as defined by T's
        /// implementation of IComparable&lt;T&gt;.Equals or object.Equals.</remarks>
        /// <param name="list">The list to search.</param>
        /// <param name="itemsToLookFor">A collection of items to search for.</param>
        /// <returns>An IEnumerable&lt;T&gt; that enumerates the indexes of items equal to
        /// any of the items in the collection <paramref name="itemsToLookFor"/>. </returns>
        public static IEnumerable<int> IndexesOfMany<T>(IList<T> list, IEnumerable<T> itemsToLookFor)
        {
            return IndexesOfMany(list, itemsToLookFor, EqualityComparer<T>.Default);
        }

        /// <summary>
        /// Enumerates the indexes of all the items in a list equal to one of several given items. A passed
        /// IEqualityComparer is used to determine equality.
        /// </summary>
        /// <param name="list">The list to search.</param>
        /// <param name="itemsToLookFor">A collection of items to search for.</param>
        ...
        /// <returns>An IEnumerable&lt;T&gt; that enumerates the indexes of items equal to
        /// any of the items in the collection <paramref name="itemsToLookFor"/>. </returns>
        public static IEnumerable<int> IndexesOfMany<T>(IList<T> list, IEnumerable<T> itemsToLookFor, IEqualityComparer<T> equalityComparer)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (itemsToLookFor == null)
        ...
        /// <summary>
        /// Enumerates the indexes of all the items in a list equal to one of several given items. The passed 
        /// BinaryPredicate is used to determine if two items are "equal".
        /// </summary>
        /// <remarks>Since an arbitrary BinaryPredicate is passed to this function, what is being removed need not be true equality. This methods finds 
        /// last item X which satisfies BinaryPredicate(X,Y), where Y is one of the items in <paramref name="itemsToLookFor"/></remarks>
        ...
        /// <param name="itemsToLookFor">A collection of items to search for.</param>
        /// <param name="predicate">The BinaryPredicate used to compare items for "equality". </param>
        /// <returns>An IEnumerable&lt;T&gt; that enumerates the indexes of items "equal" to any of the items 
        /// in the collection <paramref name="itemsToLookFor"/>, using 
        /// <paramref name="predicate"/> as the test for equality. </returns>
        public static IEnumerable<int> IndexesOfMany<T>(IList<T> list, IEnumerable<T> itemsToLookFor, BinaryPredicate<T> predicate)
        {
            if (list == null)
                throw new ArgumentNullException("list");
            if (itemsToLookFor == null)
        ...
        /// <returns><see langword="true"/> if <paramref name="collection1"/> is a subset of <paramref name="collection2"/>, considered as sets.</returns>
        ...
        /// <returns><see langword="true"/> if <paramref name="collection1"/> is a proper subset of <paramref name="collection2"/>, considered as sets.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="collection1"/> or <paramref name="collection2"/> is null.</exception>
        ...
        /// <param name="collection1">The first collection.</param>
        /// <param name="collection2">The second collection.</param>
        /// <returns><see langword="true"/> if <paramref name="collection1"/> are <paramref name="collection2"/> are disjoint, considered as sets.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="collection1"/> or <paramref name="collection2"/> is null.</exception>
        ...
        /// <param name="collection1">The first collection.</param>
        /// <param name="collection2">The second collection.</param>
        /// <returns><see langword="true"/> if <paramref name="collection1"/> are <paramref name="collection2"/> are equal, considered as sets.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="collection1"/> or <paramref name="collection2"/> is null.</exception>
        ...
        public static string ToString<T>(IEnumerable<T> collection, bool recursive, string start, string separator, string end)
        {
            ...
            foreach (T item in collection)
            {
                ...
                {
                    var genericCollection = item as IEnumerable;
                    if (recursive && genericCollection != null && !(item is string))
                        builder.Append(ToString(TypedAs<object>(genericCollection), recursive, start, separator, end));
                    else
        ...
        {
            if (myRandomGenerator == null) myRandomGenerator = new Random();
        ...
        public static T Maximum<T>(IEnumerable<T> collection, IComparer<T> comparer)
        {
            ...
            if (!foundOne)
                throw new InvalidOperationException(Resources.CollectionIsEmpty);
            else
                return maxSoFar;
        }
        ...
        public static void SortInPlace<T>(IList<T> list, IComparer<T> comparer)
        {
            ...
            if (typedArray != null)
            {
                Array.Sort(typedArray, comparer);
                return;
            }
        ...
        public static void StableSortInPlace<T>(IList<T> list, IComparer<T> comparer)
        {
            ...
            // The stable sort algorithms also uses QuickSort. An additional array of indexes (order) is
        ...
        private class LexicographicalComparerClass<T> : IComparer<IEnumerable<T>>
        {
            ...
            {
                var value = obj as LexicographicalComparerClass<T>;
                if (value != null)
                    return itemComparer.Equals(value.itemComparer);
                else
                    return false;
            }
        ...
        private class ReverseComparerClass<T> : IComparer<T>
        {
            ...
            public override bool Equals(object obj)
            {
                var objComparer = obj as ReverseComparerClass<T>;
                if (objComparer == null) return false;
                else return objComparer.Equals(objComparer.comparer);
            }
        ...
        /// </summary>
        /// <example>
        /// The following code creates a Dictionary where the keys are a collection of Properties.Resources.
        /// <code>
        ...
        /// <param name="collection">The collection to check all the items in.</param>
        /// <param name="predicate">A delegate that defines the condition to check for.</param>
        /// <returns><see langword="true"/> if the collection contains one or more items that satisfy the condition
        /// defined by <paramref name="predicate"/>. <see langword="false"/> if the collection does not contain
        ...
        /// <param name="collection">The collection to check all the items in.</param>
        /// <param name="predicate">A delegate that defines the condition to check for.</param>
        /// <returns><see langword="true"/> if all of the items in the collection satisfy the condition
        /// defined by <paramref name="predicate"/>, or if the collection is empty. <see langword="false"/> if one or more items
        ...
        public static ICollection<T> RemoveWhere<T>(ICollection<T> collection, Predicate<T> predicate)
        {
            ...
            var typedArray = collection as T[];
            if (typedArray != null)
                collection = new ArrayWrapper<T>(typedArray);

            if (collection.IsReadOnly)
                throw new ArgumentException(Resources.ListIsReadOnly, "collection");
            ...
            if (list != null)
            {
                int i = -1, j = 0;
                int listCount = list.Count;
                ICollection<T> removed = new LinkedList<T>();
                ...
                {
                    // remove items from the end.
                    var genericList = list as IList;
                    if (genericList != null && genericList.IsFixedSize)
                    {
                        // An array or similar. Null out the last elements.
                        while (i < listCount)
                            list[i++] = default(T);
        ...
        /// <param name="predicate">A delegate that defines the partitioning condition.</param>
        /// <returns>The index of the first item in the second half of the partition; i.e., the first item for
        /// which <paramref name="predicate"/> returned <c>false</c>. If the predicate was true for all items
        /// in the list, list.Count is returned.</returns>
        ...
        /// <returns><see langword="true"/> if the collections have equal items in the same order. If both collections are empty, true is returned.</returns>
        ...
        /// <returns><see langword="true"/> if <paramref name="predicate"/>returns true for each corresponding pair of
        ...
        /// <exception cref="ArgumentNullException"><paramref name="collection"/> is null.</exception>
        [SuppressMessage("Microsoft.Performance", "CA1804:RemoveUnusedLocals", Justification = "Must enumerate through collection in order to determine its length")]
        ...
        {
            if (n < 0)
                throw new ArgumentOutOfRangeException("n", n, Resources.ArgMustNotBeNegative);
            ...
            using (IEnumerator<T> sourceEnum = source.GetEnumerator())
            {
                ...
                while (sourceEnum.MoveNext())
                {
                    if (destIndex >= dest.Length)
                        throw new ArgumentException(Resources.ArrayTooSmall, "destIndex");
        ...
        public static void Copy<T>(IEnumerable<T> source, IList<T> dest, int destIndex, int count)
        {
            ...
            if (dest == null)
                throw new ArgumentNullException("dest");
            if (dest.IsReadOnly)
                throw new ArgumentException(Resources.ListIsReadOnly, "dest");
        ...
        public static void Copy<T>(IList<T> source, int sourceIndex, T[] dest, int destIndex, int count)
        {
            ...
            var typedArray = source as T[];
            if (typedArray != null)
            {
                // Array.Copy is probably faster, and also handles any overlapping issues.
                Array.Copy(typedArray, sourceIndex, dest, destIndex, count);
            }
            else
            {
                int si = sourceIndex;
