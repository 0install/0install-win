...
namespace Common.Archive
{
    ...
    class TarTestBasicFunctionality
    {
        ...
        {
            using (var extractor = Extractor.CreateExtractor("application/x-tar", new MemoryStream(_archiveData), 0, "extractedArchive"))
                extractor.RunSync();
        ...
        public void TestExtractOverwritingExistingItems()
        {
            ...
            using (var extractor = Extractor.CreateExtractor("application/x-tar", new MemoryStream(_archiveData), 0, "destination"))
                extractor.RunSync();
    ...
    class TarTestCornerCases
    {
        ...
        public void TestRejectParentDirectoryEntry()
        {
            ...
            using (var archiveStream = File.Create(Path.Combine(_sandbox.Path, "ar.tar")))
            {
                builder.GeneratePackageArchive(archiveStream);
                archiveStream.Seek(0, SeekOrigin.Begin);
                var extractor = new TarExtractor(archiveStream, 0, "extractedArchive");
                Assert.Throws<IOException>(extractor.RunSync, "TarExtractor must not accept archives with '..' as entry");
        ...
        public void TestExtractEmptyFile()
        {
            ...
            using (var archiveStream = File.Create(Path.Combine(_sandbox.Path, "ar.tar")))
            {
                ...
                const string message = "TarExtractor should correctly extract empty files in an archive";
