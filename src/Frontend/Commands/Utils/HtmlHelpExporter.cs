...
<tr>
  ...
  <td>{HtmlEncode(description)}</td>
        ...
        protected override string CommandDetails(string name, string description, string usage) => $@"<a name='{name.Replace(" ", "_")}' /><h1>{name}</h1>
<p>{HtmlEncode(description)}</p>
<p><b>Usage:</b> <code>0install {name} {usage}</code></p>";
