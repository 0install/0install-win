...
{
    /// <summary>
    /// A OneGet package provider for Zero Install. Implements OneGet's duck-typing interface.
    /// </summary>
    public class PackageProvider : Request.IPackageProvider
    {
        [PublicAPI]
        public string PackageProviderName { get { return "0install"; } }
        ...
        private void Do(Request request, Action<OneGetCommand> action, [CallerMemberName] string caller = null)
        {
            ...
            try
            {
                using (var command = new OneGetCommand(request))
                    action(command);
            }
